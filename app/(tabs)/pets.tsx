  ...
  return (
    <Screen title={pet.name} scroll={false}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 90 }}>
        <View style={s.header}>
          <View style={s.avatar}>
            {pet.photoUri ? <Image source={{ uri: pet.photoUri }} style={s.avatarImg} /> : <Icon name={speciesIcon[pet.species]} size={30} color={colors.accent} />}
          </View>
          <View style={{ flex: 1 }}>
...
const s = StyleSheet.create({
  ...
  avatar: { width: 56, height: 56, borderRadius: 14, backgroundColor: colors.accentTint, alignItems: "center", justifyContent: "center", overflow: "hidden" },
